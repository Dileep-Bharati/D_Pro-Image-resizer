...
def bulk_resize_images(form, image_files):
    # (Unchanged)
    try:
        job_id, job_folder = str(uuid.uuid4()), os.path.join('temp', str(uuid.uuid4()))
        ...
        zip_filename, zip_path = f"{job_id}.zip", os.path.join('temp', f"{job_id}.zip")
...
# --- NEW FUNCTION FOR CONVERTING ---
def convert_image(form, image_file):
    try:
        img = Image.open(image_file.stream)
        output_format = form.get('format', 'jpeg') # Get target format from form

        # Handle transparency: if converting to JPEG from a format that supports transparency (like PNG), convert to RGB.
        if output_format == 'jpeg' and img.mode in ('RGBA', 'P'):
            img = img.convert('RGB')

        # Generate a unique filename and save the converted image
        format_to_save = 'JPEG' if output_format == 'jpeg' else output_format.upper()
